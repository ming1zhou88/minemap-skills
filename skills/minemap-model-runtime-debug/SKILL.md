...
最近 master 还补了一条更细的兼容行为：

-   资产内置线框节点不只识别 `_wireframe`
-   `_crease_edge` 也被纳入内置尾缀识别

这意味着当前版本的推荐顺序应该写得更明确：

1. 资产自带 `_wireframe` / `_crease_edge` 时，优先 `showEmbeddedWireframe`
2. 资产没有内置线框时，再考虑 `generateWireframe`
3. `generateWireframe` 主要是调试/表达增强，不应默认全开
